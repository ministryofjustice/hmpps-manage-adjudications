...
context('Print completed DIS forms', () => {
  ...
  it('has all the correct data in the table', () => {
    ...
    printCompletedDISFormsPage
      ...
      .then($data => {
        ...
        expect($data.get(1).innerText).to.contain('7 December 2022 - 10:00')
        expect($data.get(2).innerText).to.contain('MDI-MCASU')
        expect($data.get(3).innerText).to.contain('-')
        expect($data.get(4).innerText).to.contain('Not issued')
        expect($data.get(5).innerText).to.contain('Print notice of report')
        expect($data.get(6).innerText).to.contain('Smith, James - G7234VB')
        expect($data.get(7).innerText).to.contain('6 December 2022 - 10:00')
        expect($data.get(8).innerText).to.contain('MDI-RECP')
        expect($data.get(9).innerText).to.contain('ACCT OPEN')
        expect($data.get(9).innerText).to.contain('PEEP')
        expect($data.get(9).innerText).to.contain('CSIP')
        expect($data.get(9).innerText).to.contain('PREGNANT')
        expect($data.get(10).innerText).to.contain('Issued')
  ...
  it('should handle if there is prisoner information missing', () => {
    ...
    printCompletedDISFormsPage
      ...
      .find('td')
      .then($data => {
        expect($data.get(0).innerText).to.contain('Unknown - P3785CP')
        expect($data.get(1).innerText).to.contain('7 December 2022 - 10:00')
