...
context('Inad refers to gov after hearing', () => {
  ...
  describe('---', () => {
    it('-', () => {
      ...
      nextStepsInadPage.nextStepRadioButtons().find('input[value="refer_gov"]').click()
      ...
      hearingTabPage.nextStepReferralOutcomeButton().click()
      const nextStepsGovPage = Page.verifyOnPage(NextStepsGovPage)
      nextStepsGovPage.nextStepRadioButtons().find('input[value="schedule_hearing"]').click()
      nextStepsGovPage.submitButton().click()
      ...
      hearingTabPage
        ...
        .find('dd')
        .then($summaryData => {
          expect($summaryData.get(0).innerText).to.contain('This is the reason for referring to the governor')
