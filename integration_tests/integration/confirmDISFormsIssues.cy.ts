  ...
  it('should have the required elements - reports present but not issued', () => {
    ...
    confirmDISFormsIssued
      ...
      .then($data => {
        ...
        expect($data.get(7).innerText).to.contain('23456')
        expect($data.get(8).innerText).to.contain('Tovey, Peter - P3785CP')
        expect($data.get(9).innerText).to.contain('6 December 2022 - 12:10')
        expect($data.get(10).innerText).to.contain('MDI-MCASU')
