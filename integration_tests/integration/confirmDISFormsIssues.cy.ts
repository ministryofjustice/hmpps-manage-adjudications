...
const prisoners = [
  ...
  testData.simplePrisoner('P3785CP', 'PETER', 'TOVEY', 'MDI-2-3-009'),
]
...
context('Confirm DIS forms have been issued', () => {
  ...
  it('should have the required elements - reports present but not issued', () => {
    ...
    confirmDISFormsIssued
      ...
      .then($headers => {
        ...
        expect($headers.get(8).innerText).to.contain('MDI-2-3-009')
  ...
  it.skip('should filter on the parameters given - dates and location', () => {
    const adjudicationResponse = [
      testData.completedAdjudication(
        12345,
        'G7234VB',
