...
context('View scheduled hearings', () => {
  ...
  it('should have the correct details showing in the table', () => {
    ...
    viewScheduledHearingsPage
      ...
      .then($data => {
        ...
        expect($data.get(1).innerText).to.contain('5 November 2025 - 14:00')
        expect($data.get(2).innerText).to.contain('Kirk, Rosemary - P3785CP')
        expect($data.get(3).innerText).to.contain('Adjourned')
        ...
        expect($data.get(7).innerText).to.contain('5 November 2025 - 13:00')
        expect($data.get(8).innerText).to.contain('Salgado, Emmanuel - G6345BY')
        expect($data.get(9).innerText).to.contain('Charge proved')
  ...
  it('should accept a user-chosen date (using datepicker)', () => {
    ...
    viewScheduledHearingsPage
      ...
      .find('td')
      .then($data => {
        expect($data.get(0).innerText).to.contain('678912')
        expect($data.get(1).innerText).to.contain('6 November 2025 - 11:00')
        expect($data.get(2).innerText).to.contain('Barnett, Mallory - M1876DT')
        expect($data.get(3).innerText).to.contain('Adjourned')
        expect($data.get(4).innerText).to.contain('View hearing')
        expect($data.get(5).innerText).to.contain('View report')
        expect($data.get(6).innerText).to.contain('567894')
        expect($data.get(7).innerText).to.contain('6 November 2025 - 10:00')
        expect($data.get(8).innerText).to.contain('Whitehead, Yunus - D9543CY')
        expect($data.get(9).innerText).to.contain('Dismissed')
        expect($data.get(10).innerText).to.contain('View hearing')
        expect($data.get(11).innerText).to.contain('View report')
      })
  })
  ...
  it('should handle if some prisoner information is missing', () => {
    ...
    viewScheduledHearingsPage
      ...
      .then($data => {
        expect($data.get(0).innerText).to.contain('123456')
        expect($data.get(1).innerText).to.contain('5 November 2025 - 14:00')
        expect($data.get(2).innerText).to.contain('Unknown - P3785CP')
        expect($data.get(3).innerText).to.contain('Adjourned')
        expect($data.get(4).innerText).to.contain('View hearing')
        expect($data.get(5).innerText).to.contain('View report')
        expect($data.get(6).innerText).to.contain('123456')
