...
context('Check Your Answers', () => {
  ...
  it('should contain the required page elements', () => {
    cy.visit(adjudicationUrls.checkYourAnswers.urls.review(3456))
  ...
  it('should go to the completion page (changed - reviewer) if the user submits changes to the report', () => {
    ...
    cy.location().should(loc => {
      expect(loc.pathname).to.eq(adjudicationUrls.confirmedOnReport.urls.reviewerView(234))
    })
  })
  it('should go to the prisoner report page if the user cancels', () => {
    ...
    cy.location().should(loc => {
      expect(loc.pathname).to.eq(adjudicationUrls.prisonerReport.urls.review(234))
    })
  })
  it('should go to the incident details page if the incident details change link is clicked', () => {
    ...
    cy.location().should(loc => {
      ...
      expect(loc.href).to.include(`?referrer=${adjudicationUrls.checkYourAnswers.urls.review(3456)}`)
    })
  })
