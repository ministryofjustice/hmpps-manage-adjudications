...
class PageOptions {
  constructor(private readonly pageTypeBasic: PageRequestType) {}

  isEdit(): boolean {
    return this.pageTypeBasic === PageRequestType.EDIT || this.pageTypeBasic === PageRequestType.EDIT_SUBMITTED
  }

  isPreviouslySubmitted(): boolean {
    return this.pageTypeBasic === PageRequestType.EDIT_SUBMITTED
  }
}

export default class IncidentDetailsPageBasic {
  pageOptions: PageOptions

  constructor(
    pageTypeBasic: PageRequestType,
    private readonly placeOnReportService: PlaceOnReportService,
    private readonly locationService: LocationService
  ) {
    this.pageOptions = new PageOptions(pageTypeBasic)
  }
  ...
  submit = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      //   incidentDetailsToSave.currentIncidentRoleSelection
      // )
      return redirectToPrisonRuleSelection(res, newDraftData.draftAdjudication.id)
    } catch (postError) {
      if (this.pageOptions.isEdit()) {
...
const redirectToPrisonRuleSelection = (res: Response, draftId: number) => {
