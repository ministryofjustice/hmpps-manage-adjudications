  ...
  it('should load the correct details', () => {
    return request(app)
      ...
      .expect(response => {
        ...
        expect(response.text).toContain('Date of discovery:</span> 15/11/2021 - 11:45')
        expect(response.text).toContain('Happened at: </span> Moorland (HMP)')
        expect(response.text).toContain('MDI-100000')
        expect(response.text).toContain('Awaiting review')
        expect(response.text).toContain('Date of discovery:</span> 20/11/2021 - 13:30')
        expect(response.text).toContain('Happened at: </span> Moorland (HMP)')
      })
  })
})
